 ...
 * 
 * 提供统一的订阅内容获取、解析和展开功能。
 * 被 subscription-fetch-service.ts 和 compat-nav-sub-service.ts 共同使用。
 */
 ...
 * - 大小限制（5MB）
 */
export async function fetchSubscription(env: SubscriptionEnv, rawUrl: string, depth = 0): Promise<{ text: string }> {
  // 检查重定向次数
